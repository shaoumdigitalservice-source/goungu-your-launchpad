...
interface Props {
  ...
  onAccepted?: (updated: CandidatureApi) => void;
}

export default function AcceptCandidatureDialog({
  open,
  candidature,
  onOpenChange,
  onAccepted,
  ...
  const handleConfirm = async () => {
    ...
    setError(null);
    try {
      const updated = await acceptCandidature(candidature.id);
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <DialogFooter className="gap-2 sm:gap-2">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-medium disabled:opacity-60"
          >
            {loading ? (
